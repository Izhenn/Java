...
class GamePanel extends JPanel implements Runnable{
	...
	private ImageIcon snakeheadIcon_right = new ImageIcon("snakehead_right.png");
	private ImageIcon snakeheadIcon_left = new ImageIcon("snakehead_left.png");
	private ImageIcon snakeheadIcon_up = new ImageIcon("snakehead_up.png");
	private ImageIcon snakeheadIcon_down = new ImageIcon("snakehead_down.png");

	private ImageIcon cakeIcon= new ImageIcon("cake.jpg");
	...
	protected void paintComponent(Graphics g){
		...
		if(isFirstRun){
			...
			g.fillRect(xCentre - HALF_SIDE, yCentre - HALF_SIDE, 600, 600);
			g.setColor(Color.BLACK);
            for(int i=1;i<=30;i++){
                g.drawLine(xCentre - HALF_SIDE,yCentre - HALF_SIDE+20*i,xCentre - HALF_SIDE+600,yCentre - HALF_SIDE+20*i);
                g.drawLine(xCentre - HALF_SIDE+20*i,yCentre - HALF_SIDE,xCentre - HALF_SIDE+20*i,yCentre - HALF_SIDE+600);
            }
			...
			Image snakehead_right = snakeheadIcon_right.getImage();
			g.drawImage(snakehead_right, snake.getHead().getX(), snake.getHead().getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH, this);
			// int headDirection = snake.getDirection();
			// ImageIcon currentHeadIcon = snake.getHeadIcons()[headDirection - 1];
			// Image snakehead = currentHeadIcon.getImage();
			// g.drawImage(snakehead, snake.getHead().getX(), snake.getHead().getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH, this);
			...
			Image cake=cakeIcon.getImage();
			g.drawImage(cake, dessert.getX(), dessert.getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH,null);
			// g.drawImage(cake, dessert.getX(), dessert.getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH);
			...
			case Snake.DIRECTION_UP:
				Image snakehead_up = snakeheadIcon_up.getImage();
				g.drawImage(snakehead_up, snake.getHead().getX(), snake.getHead().getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH, null);
			case Snake.DIRECTION_DOWN:
				Image snakehead_down = snakeheadIcon_down.getImage();
				g.drawImage(snakehead_down, snake.getHead().getX(), snake.getHead().getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH, null);
			case Snake.DIRECTION_LEFT:
				Image snakehead_left = snakeheadIcon_left.getImage();
				g.drawImage(snakehead_left, snake.getHead().getX(), snake.getHead().getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH, null);
			case Snake.DIRECTION_RIGHT:
				Image snakehead_right = snakeheadIcon_right.getImage();
				g.drawImage(snakehead_right, snake.getHead().getX(), snake.getHead().getY(), PER_UNIT_LENGTH, PER_UNIT_LENGTH, null);
			default:
		}
...
class Snake{//蛇類
	...
	private ImageIcon[] headIcons = new ImageIcon[4];

	private Dot head = new Dot();
	private LinkedList<Dot> body = new LinkedList<Dot>();

	public Snake(){
		// headIcons[DIRECTION_UP - 1] = new ImageIcon("snakehead_up.png");
        // headIcons[DIRECTION_DOWN - 1] = new ImageIcon("snakhead_down.png");
        // headIcons[DIRECTION_LEFT - 1] = new ImageIcon("snakhead_left.png");
        // headIcons[DIRECTION_RIGHT - 1] = new ImageIcon("snakhead_right.png");
	}
	// public ImageIcon[] getHeadIcons() {
    //     return headIcons;
    // }
